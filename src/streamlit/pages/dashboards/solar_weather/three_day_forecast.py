...
import pytz
from src.utils.models import ThreeDayForecast


def convert_time_periods_to_local(time_periods, target_timezone='America/New_York'):
    """
...
def create_kp_table(data: ThreeDayForecast, use_local_time=False, timezone_info=('UTC', 'UTC')):
    ...
    header_values = [f"Time Period ({timezone_short if use_local_time and timezone_str != 'UTC' else 'UTC'})"] + list(df.columns)
